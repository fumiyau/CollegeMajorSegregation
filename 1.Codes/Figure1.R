#=====================================================================
# 2020/06/29
# College major segregation (Figure 1)
...
setwd("CollegeMajorSegregation/2.Data/")
...
df <- read_xlsx("学校基本調査進学率.xlsx",skip=10) %>% 
...
All_2015 <- lapply(1:4,function(i){
  lapply(2015:2019,function(j){
    read_excel(paste("NameEdited/Enrolls/",j,".xlsx",sep=""), sheet = i,skip=4) %>%
      dplyr::select(2,5,6) %>% 
      mutate(year = j,
             inst = i)
  })
...
df_pri <- read_xlsx("W3.xlsx",skip=6) %>% 
...
ggsave(height=6,width=9,dpi=200, filename="../3.Results/Figure1.pdf",  family = "Helvetica")
